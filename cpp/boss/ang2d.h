...
//	if ((p.decmax < -90) || (p.decmax > 90)) throw ("RABounds exception thrown");
//	if (p.decmax < DecBounds.first) throw("Dec Bounds exception thrown");
//
//	// theta data validity
...
//
//	// RA
//	if ((p.ramin < 0) || (p.ramin > 360.0)) throw ("RABounds exception thrown");
//	if ((p.ramax < 0) || (p.ramax > 360.0)) throw ("RABounds exception thrown");
//	if (p.ramax < p.ramin) p.ramax += 360; // Wrap around
//
//
//	// Survey Bounds
//	double cth1 = sin(DecBounds.first * D2R); // Cos theta_1
//	double dcth = sin(p.decmax*D2R) - cth1; // Cos theta_2 - Cos theta_1
//	double phi1 = p.ramin * D2R; // phi_1
//	double dphi = (p.ramax - p.ramin) * D2R; // phi_2 - phi1
//
//	// Scatter bounds
